...
import pdb # pdb.set_trace() (REMOVE AFTER TESTING) #JLR
...
#%%
    ...
    for file in files:
        ...
        # dset_tdew = xr.open_dataset(file.replace('ta','td'),diskless=True)  # JLR!!!
        ...
        # fill the values with NA where condition is false, where there should be NaN
            ...
            time_value = (file_dates_dly[ii] - pd.to_datetime(datetime(1950, 1, 1))).total_seconds() / 86400 #JLR
            index = np.where(out_dates_dly==file_dates_dly[ii])[0][0]
            ...
            # JLR: WRONG! ALL VARIABLES COVER EXACT SAME PERIOD (THE SAME HOURLY GRIDS ARE DOWNLOADED FROM CDS).
            # ONLY DIFFERENCE IS THAT SOME ARE AVERAGED, OTHERS ACCUMULATED.
            # AS LISFLOOD USES END-OF-TIMESTEP CONVENTION, THE TIME VALUES OF ALL VARS NEED TO BE SHIFTED +1
            # EDIT: ERA5land HAS DIFFERENT CONVENTIONS FOR HOURLY VALUES. THEY ARE ACCUMULATED OVER THE DAY:
            # VALUES AT 00:00 REPRESENT FULL ACCUMULATION OF THE PREVIOUS DAY, THEN AT 01:00 IT IS RESET TO ZERO
